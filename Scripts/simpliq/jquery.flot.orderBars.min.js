(function ($) {
    function init(plot) {
        var orderedBarSeries; var nbOfBarsToOrder; var borderWidth; var borderWidthInXabsWidth; var pixelInXWidthEquivalent = 1; function reOrderBars(plot, serie, datapoints) {
        ...
        function getXabsMinMaxValues(series) {
            var minMaxValues = new Array(); for (var i = 0; i < series.length; i++) { minMaxValues[0] = series[i].data[0][0]; minMaxValues[1] = series[i].data[series[i].data.length - 1][0]; }
            return minMaxValues;
        }
        ...
        function shiftPoints(datapoints, serie, dx) {
            var ps = datapoints.pointsize; var points = datapoints.points; var j = 0; for (var i = 0; i < points.length; i += ps) { points[i] += dx; serie.data[j][3] = points[i]; j++; }
            return points;
        }
